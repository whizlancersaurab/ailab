...
import { AiDeviceTypeCountWithDataForSchoolDas} from "../../../../service/api.ts";
import { Spinner } from "../../../../spinner.tsx";

const AiDeviceTypeListForSchoolDas = () => {
    const route = all_routes
    const { schoolId } = useParams()
    console.log("hhfdsjkh")
    const [deviceTypeList, setDeviceTypeList] = useState<any>([])
    const [loading, setLoading] = useState<boolean>(false)
    const fetchDeviceTypesList = async (schoolId: number) => {
        setLoading(true)

        try {
            const { data } = await AiDeviceTypeCountWithDataForSchoolDas(schoolId)
    ...
    useEffect(() => {
        if (schoolId) {
            fetchDeviceTypesList(Number(schoolId))
        }
...
export default AiDeviceTypeListForSchoolDas;