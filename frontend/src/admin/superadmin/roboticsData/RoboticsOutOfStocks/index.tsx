...
import { roboOutOfStockDevicesForSchoolDas } from "../../../../service/api";
...
const RoboOutOfStockDevice = () => {
    ...
    const fetchdevices = async (schoolId: number) => {
        setLoading(true);
        try {
            const { data } = await roboOutOfStockDevicesForSchoolDas(schoolId);
            if (data?.success) {
...
export default RoboOutOfStockDevice;
